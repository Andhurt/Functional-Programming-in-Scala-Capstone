...
object Visualization {
  ...
  def idw(sample: Iterable[(Location, Temperature)], x: Location, p: Temperature) = {

    @tailrec
    def recIdw(values: Iterator[(Location, Temperature)], sumVals: Temperature, sumWeights: Temperature): Temperature = {
      values.next match {
        case (xi, ui) => {
  ...
  def interpolateColor2(sortedPoints: Array[(Temperature, Color)], value: Temperature): Color = {
